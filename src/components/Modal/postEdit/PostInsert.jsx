...
const PostInsert = () => {
  ...
  return (
    <>
      ...
      }} className='hidden' name='files' id='files' />
      {
              ...
              {formik.values.files.map((file, index) => {
                ...
                return (
                  <div key={index} className='embla__slide relative p-4'>
                    {fileType === 'Photo' && <Image src={URL.createObjectURL(file)} className="z-0 w-full h-full object-cover" />}
                    {fileType === 'Video' && (
                      <video className="z-0 w-full h-full object-cover" controls preload="none" poster="/video.svg">
                        <source src={URL.createObjectURL(file)} type="video/mp4" />
                      </video>
                    )}
                    {fileType === 'Docs' && (
                      <Card isBlurred className="z-0 w-full h-full object-cover FLEX-CENTER gap-10 my-10 py-10 bg-green-500/10">
...
<Button htmlFor='files' as='label' color='success' variant='flat' className="w-full h-full  p-0 FLEX-CENTER" isIconOnly>
                            <FaUpload size={60} />
                        </Button>
</Card>
        )
